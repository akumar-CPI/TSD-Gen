...
function tableFromRows(label, rows) {
  ...
  if (!rows || !rows.length) {
    out.push(makeTable(cols, [["\u2014", "\u2014", "\u2014", "\u2014", "\u2014", "\u2014"]], [12, 20, 12, 14, 30, 12]));
    out.push(spacer());
    return out;
  }
...
function renderInstance(step, content, parsed) {
  ...
  if (type === "Content Modifier") {
    out.push(...tableFromRows("Message Header \u2014 Headers", step.headerTable));
    out.push(...tableFromRows("Exchange Property \u2014 Properties", step.propertyTable));
    const bodyType = step.props?.bodyType;
    const bodyValue = step.props?.body;
    out.push(h4("Message Body"));
    out.push(makeTable(["Field", "Value"], [
      ["Type", bodyType || "\u2014"],
      ["Body", bodyValue || "\u2014"],
    ], [30, 70]));
    out.push(spacer());
  ...
  out.push(h4("Configuration"));
  out.push(makeTable(["Property", "Value"], rows.length ? rows : [["\u2014", "\u2014"]], [40, 60]));
  out.push(spacer());
  return out;
}
...
async function build({ parsed, content, diagrams, docTitle, documentMetadata = {} }) {
  ...
  children.push(makeTable(
    ["Name", "Type", "Action"],
    scriptEntries.length ? scriptEntries.map((s) => [s, "Script", "Included in iFlow"]) : [["\u2014", "\u2014", "\u2014"]],
    [40, 30, 30]
  ));
  children.push(spacer());
  ...
  children.push(makeTable(
    ["Name", "Value"],
    parsed.parametersDefined && parsed.parametersDefined.length ? parsed.parametersDefined.map((p) => [p.name, "\u2014"]) : [["\u2014", "\u2014"]],
    [50, 50]
  ));
  children.push(spacer(), spacer());
  ...
  children.push(makeTable(
    ["Name", "Type"],
    parsed.participants.length ? parsed.participants.map((p) => [p.name || "\u2014", p.type || "\u2014"]) : [["\u2014", "\u2014"]],
    [50, 50]
  ));
  children.push(spacer());
  ...
  children.push(makeTable(
    ...
    securityMaterialRows.length ? securityMaterialRows : [["\u2014", "\u2014", "\u2014", "\u2014", "\u2014"]],
    [24, 24, 20, 16, 16]
  ));
  children.push(spacer());
  ...
  children.push(makeTable(
    ...
    content.testPlanRows && content.testPlanRows.length ? content.testPlanRows.map((r) => [r[0], r[1], "TBD"]) : [["\u2014", "\u2014", "\u2014"]],
    [50, 30, 20]
  ));
  children.push(spacer(), spacer());

  children.push(h2("Appendix A: Automated Review Findings"));
  children.push(body("Supplementary automated review, derived from the parsed iFlow \u2014 not part of the standard TSD template, kept for convenience.", { italics: true }));
  children.push(spacer());
  children.push(makeTable(
    ["Severity", "Area", "Finding", "Recommendation"],
    content.reviewRows && content.reviewRows.length ? content.reviewRows : [["\u2014", "\u2014", "No findings detected.", "\u2014"]],
    [10, 14, 38, 38]
  ));
  children.push(spacer());
